...
1. 登录平台，进入运营总览。
2. 展示设备总数、异常设备、待处理报警、风险设备排行。
3. 进入设备中心，打开 DEV-005，说明设备画像、报警历史、维修记忆。
4. 进入智能诊断，选择 DEV-005，输入“分析设备振动异常原因以及处理方法”。
5. 展示 Agent 执行流程：
   - 任务规划
   - 数据获取
   - 知识检索
...
6. 展示 Report V2：
   ...
   - 处理方案
   - 知识库引用
7. 进入维修闭环，说明 AI 建议如何转化为现场处理记录和维护记忆。
8. 进入系统管理，展示 RBAC、健康检查、LLM real mode、Reranker 状态。
...
基于 FastAPI、React、PostgreSQL、ChromaDB 和大模型 API 构建企业级工业设备智能运维平台，实现设备状态感知、报警分析、Agent Tool Calling、RAG 知识库检索、Reranker 二阶段重排、Report V2 结构化诊断报告、JWT/RBAC 权限控制和 Docker 化部署。系统通过 Agent Orchestrator 将用户请求拆解为任务规划、设备数据获取、报警查询、知识检索、风险分析和报告生成等步骤，确保诊断结论基于真实设备数据和可追溯知识来源。
```

面试讲解重点：

- 不只是调用大模型，而是通过 Agent 编排完成“理解、规划、执行、检索、推理、报告生成”。
- RAG 不是简单文档搜索，而是结合设备类型、报警码、运行参数和历史案例进行上下文检索。
- Reranker 用于提升知识引用准确率，避免低相关 chunk 进入诊断报告。
- Report V2 是前端唯一可信展示来源，前端不编造风险、原因和维修建议。
- Docker Compose 支持后端、前端、PostgreSQL、ChromaDB 一键启动，具备企业项目工程化能力。
...
- PostgreSQL：healthy
- ChromaDB：running
- `/health`：database connected、vector_db connected、LLM real mode、Reranker enabled
